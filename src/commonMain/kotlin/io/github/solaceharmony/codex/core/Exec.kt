...
data class StdoutStream(
        val subId: String,
        val callId: String,
        val txEvent: SendChannel<io.github.solaceharmony.codex.protocol.Event>
)
...
class Exec {
    ...
    /**
     * Simple command execution for internal tools like git. Returns a basic result with exit code,
     * stdout, and stderr.
     */
    suspend fun executeCommand(
        ...
        val result =
                execute(params = params, sandboxPolicy = SandboxPolicy.ReadOnly, sandboxCwd = cwd)
    ...
    private suspend fun exec(
        ...
        val splitCommand =
                command.splitFirst() ?: throw IllegalArgumentException("command args are empty")
    ...
    private suspend fun consumeTruncatedOutput(
                        ...
                        try {
                            val result =
                                    kotlinx.coroutines.withTimeout(
                                            expiration.duration.inWholeMilliseconds
                                    ) { process.onAwait() }
                            Pair(result, false)
        ...
        val stdout =
                StreamOutput(text = process.stdout ?: byteArrayOf(), truncatedAfterLines = null)
        val stderr =
                StreamOutput(text = process.stderr ?: byteArrayOf(), truncatedAfterLines = null)

        // Aggregate output
        val aggregatedOutput =
                StreamOutput(
                        text = (process.stdout
                                        ?: byteArrayOf()) + (process.stderr ?: byteArrayOf()),
    ...
    private suspend fun finalizeExecResult(
        ...
        if (isLikelySandboxDenied(sandboxType, execOutput)) {
            ...
            throw io.github.solaceharmony.codex.core.CodexException(
                    CodexErr.Sandbox(io.github.solaceharmony.codex.core.SandboxErr.Denied(execOutput))
            )
        }
