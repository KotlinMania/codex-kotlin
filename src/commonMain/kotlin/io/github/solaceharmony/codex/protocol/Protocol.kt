...
@Serializable data class Submission(val id: String, val op: Op)

/** Submission operation - tagged union for all user operations. */
@Serializable
sealed class Op {
    @Serializable @SerialName("interrupt") object Interrupt : Op()

    @Serializable
    @SerialName("user_input")
    data class UserInput(val items: List<io.github.solaceharmony.codex.protocol.UserInput>) : Op()

    @Serializable
    @SerialName("user_turn")
    data class UserTurn(
            val items: List<io.github.solaceharmony.codex.protocol.UserInput>,
            val cwd: String,
            ...
            @SerialName("final_output_json_schema") val finalOutputJsonSchema: JsonElement? = null
    ) : Op()
    ...
    @Serializable @SerialName("add_to_history") data class AddToHistory(val text: String) : Op()

    @Serializable
    @SerialName("get_history_entry_request")
    data class GetHistoryEntryRequest(val offset: Long, @SerialName("log_id") val logId: Long) :
            Op()

    @Serializable @SerialName("list_mcp_tools") object ListMcpTools : Op()

    @Serializable @SerialName("list_custom_prompts") object ListCustomPrompts : Op()

    @Serializable @SerialName("compact") object Compact : Op()

    @Serializable @SerialName("undo") object Undo : Op()

    @Serializable
    @SerialName("review")
    data class Review(@SerialName("review_request") val reviewRequest: ReviewRequest) : Op()

    @Serializable @SerialName("shutdown") object Shutdown : Op()

    @Serializable
    @SerialName("run_user_shell_command")
    data class RunUserShellCommand(val command: String) : Op()
}
...
sealed class SandboxPolicy {
    @Serializable @SerialName("danger-full-access") object DangerFullAccess : SandboxPolicy()

    @Serializable @SerialName("read-only") object ReadOnly : SandboxPolicy()
    ...
    fun hasFullNetworkAccess(): Boolean =
            when (this) {
                is DangerFullAccess -> true
                is ReadOnly -> false
        ...
        return when (this) {
            ...
            is WorkspaceWrite -> {
                ...
                roots.map { root ->
                    ...
                    WritableRoot(root = root, readOnlySubpaths = listOf(gitPath))
                }
            }
        }
    }
...
data class WritableRoot(
        val root: String,
        @SerialName("read_only_subpaths") val readOnlySubpaths: List<String>
) {
...
@Serializable data class Event(val id: String, val msg: EventMsg)

/** Response event from the agent - tagged union with 40+ event types. */
@Serializable
sealed class EventMsg {
    @Serializable @SerialName("error") data class Error(val payload: ErrorEvent) : EventMsg()

    @Serializable @SerialName("warning") data class Warning(val payload: WarningEvent) : EventMsg()
    ...
    data class AgentReasoningRawContentDelta(val payload: AgentReasoningRawContentDeltaEvent) :
            EventMsg()

    @Serializable
    @SerialName("agent_reasoning_section_break")
    data class AgentReasoningSectionBreak(val payload: AgentReasoningSectionBreakEvent) :
            EventMsg()
    ...
    @Serializable @SerialName("shutdown_complete") object ShutdownComplete : EventMsg()
...
sealed class CodexErrorInfo {
    ...
    @Serializable @SerialName("usage_limit_exceeded") object UsageLimitExceeded : CodexErrorInfo()

    @Serializable
    @SerialName("http_connection_failed")
    data class HttpConnectionFailed(
            @SerialName("http_status_code") val httpStatusCode: Int? = null
    ) : CodexErrorInfo()
    ...
    @Serializable @SerialName("unauthorized") object Unauthorized : CodexErrorInfo()

    @Serializable @SerialName("bad_request") object BadRequest : CodexErrorInfo()

    @Serializable @SerialName("sandbox_error") object SandboxError : CodexErrorInfo()
    ...
    @Serializable @SerialName("other") object Other : CodexErrorInfo()
}
...
data class TaskCompleteEvent(
        @SerialName("last_agent_message") val lastAgentMessage: String? = null
)
...
data class TokenUsage(
        ...
        @SerialName("reasoning_output_tokens") val reasoningOutputTokens: Long = 0,
...
data class TokenUsageInfo(
        @SerialName("total_token_usage") val totalTokenUsage: TokenUsage,
        @SerialName("last_token_usage") val lastTokenUsage: TokenUsage,
        @SerialName("model_context_window") val modelContextWindow: Long? = null
) {
...
@Serializable
data class FinalOutput(@SerialName("token_usage") val tokenUsage: TokenUsage) {
...
data class TokenCountEvent(
        ...
        @SerialName("rate_limits") val rateLimits: RateLimitSnapshot? = null
)
...
data class RateLimitWindow(
        @SerialName("used_percent") val usedPercent: Double,
...
@Serializable data class AgentMessageEvent(val message: String)

@Serializable data class UserMessageEvent(val message: String, val images: List<String>? = null)

@Serializable data class AgentMessageDeltaEvent(val delta: String)

@Serializable data class AgentReasoningEvent(val text: String)

@Serializable data class AgentReasoningRawContentEvent(val text: String)

@Serializable data class AgentReasoningRawContentDeltaEvent(val delta: String)
...
@Serializable data class AgentReasoningDeltaEvent(val delta: String)
...
@Serializable data class WebSearchBeginEvent(@SerialName("call_id") val callId: String)

@Serializable
data class WebSearchEndEvent(@SerialName("call_id") val callId: String, val query: String)
...
@Serializable
data class ViewImageToolCallEvent(@SerialName("call_id") val callId: String, val path: String)
...
data class ExecCommandOutputDeltaEvent(
    ...
    override fun hashCode(): Int =
            31 * (31 * callId.hashCode() + stream.hashCode()) + chunk.contentHashCode()
}

@Serializable data class BackgroundEventEvent(val message: String)

@Serializable data class DeprecationNoticeEvent(val summary: String, val details: String? = null)

@Serializable data class UndoStartedEvent(val message: String? = null)

@Serializable data class UndoCompletedEvent(val success: Boolean, val message: String? = null)
...
@Serializable data class TurnDiffEvent(@SerialName("unified_diff") val unifiedDiff: String)
...
@Serializable data class McpStartupUpdateEvent(val server: String, val status: McpStartupStatus)
...
@Serializable data class McpStartupFailure(val server: String, val error: String)
...
@Serializable data class TurnAbortedEvent(val reason: TurnAbortReason)
...
sealed class InitialHistory {
    ...
    fun getRolloutItems(): List<RolloutItem> =
            when (this) {
                is New -> emptyList()
    ...
    fun getEventMsgs(): List<EventMsg>? =
            when (this) {
                is New -> null
                is Resumed ->
                        payload.history.mapNotNull {
                            if (it is RolloutItem.EventMsgItem) it.payload else null
                        }
                is Forked ->
                        items.mapNotNull {
                            if (it is RolloutItem.EventMsgItem) it.payload else null
...
@Serializable data class SessionMetaLine(val meta: SessionMeta, val git: GitInfo? = null)

@Serializable
sealed class RolloutItem {
    @Serializable
    @SerialName("session_meta")
    data class SessionMetaItem(val payload: SessionMetaLine) : RolloutItem()
    @Serializable
    @SerialName("response_item")
    data class ResponseItemHolder(val payload: ResponseItem) : RolloutItem()
    @Serializable
    @SerialName("compacted")
    data class Compacted(val payload: CompactedItem) : RolloutItem()
    @Serializable
    @SerialName("turn_context")
    data class TurnContextHolder(val payload: TurnContextItem) : RolloutItem()
    @Serializable
    @SerialName("event_msg")
    data class EventMsgItem(val payload: EventMsg) : RolloutItem()
}
...
@Serializable data class RolloutLine(val timestamp: String, val item: RolloutItem)
...
@Serializable data class ReviewLineRange(val start: Int, val end: Int)

@Serializable
data class ExitedReviewModeEvent(
        @SerialName("review_output") val reviewOutput: ReviewOutputEvent? = null
)
 ...
 */
@Serializable data class StreamInfoEvent(val message: String)

// ========== Item Events ==========

@Serializable data class RawResponseItemEvent(val item: ResponseItem)
...
data class ItemStartedEvent(
    ...
    fun asLegacyEvents(showRawAgentReasoning: Boolean): List<EventMsg> =
            when (val i = item) {
                is TurnItem.WebSearch ->
                        listOf(EventMsg.WebSearchBegin(WebSearchBeginEvent(callId = i.item.id)))
                else -> emptyList()
            }
}
...
data class ItemCompletedEvent(
    ...
    fun asLegacyEvents(showRawAgentReasoning: Boolean): List<EventMsg> =
            item.asLegacyEvents(showRawAgentReasoning)
}
...
data class AgentMessageContentDeltaEvent(
    ...
    fun asLegacyEvents(): List<EventMsg> =
            listOf(EventMsg.AgentMessageDelta(AgentMessageDeltaEvent(delta = delta)))
}
...
data class ReasoningContentDeltaEvent(
    ...
    fun asLegacyEvents(): List<EventMsg> =
            listOf(EventMsg.AgentReasoningDelta(AgentReasoningDeltaEvent(delta = delta)))
}
    ...
    fun asLegacyEvents(): List<EventMsg> =
            listOf(
                    EventMsg.AgentReasoningRawContentDelta(
                            AgentReasoningRawContentDeltaEvent(delta = delta)
                    )
            )
}
