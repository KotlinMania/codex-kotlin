...
import io.github.solaceharmony.codex.api.common.Prompt
...
import io.github.solaceharmony.codex.api.provider.WireApi
import io.github.solaceharmony.codex.api.requests.ChatRequest
...
class ChatClient<A : AuthProvider>(
    ...
    suspend fun streamPrompt(
        ...
        sessionSource: io.github.solaceharmony.codex.protocol.SessionSource?,
    ...
    suspend fun stream(
        ...
        return streaming.stream(path(), body, configureExtraHeaders) { client, requestConfig, idleTimeout, telemetry ->
            io.github.solaceharmony.codex.api.sse.spawnChatStream(client, requestConfig, idleTimeout, telemetry)
        }
    }
}
        ...
        while (true) {
            ...
            when (event) {
                is ResponseEvent.OutputItemDone -> {
                    val item = event.item
                    val isAssistantMessage = item is io.github.solaceharmony.codex.protocol.ResponseItem.Message && item.role == "assistant"
                        ...
                        when (mode) {
                            AggregateMode.AggregatedOnly -> {
                                ...
                                if (cumulative.isEmpty()) {
                                    item.content.firstOrNull { it is io.github.solaceharmony.codex.protocol.ContentItem.OutputText }
                                        ?.let { contentItem ->
                                            if (contentItem is io.github.solaceharmony.codex.protocol.ContentItem.OutputText) {
                ...
                is ResponseEvent.Completed -> {
                    ...
                    if (cumulativeReasoning.isNotEmpty()) {
                        val aggregatedReasoning = io.github.solaceharmony.codex.protocol.ResponseItem.Reasoning(
                            id = "",
                            summary = emptyList(),
                            content = listOf(io.github.solaceharmony.codex.protocol.ReasoningItemContent.ReasoningText(
                    ...
                    if (cumulative.isNotEmpty()) {
                        val aggregatedMessage = io.github.solaceharmony.codex.protocol.ResponseItem.Message(
                            role = "assistant",
                            content = listOf(io.github.solaceharmony.codex.protocol.ContentItem.OutputText(text = cumulative.toString())),
                            id = null
                        )
