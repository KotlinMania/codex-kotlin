...
class McpHandler : ToolHandler {
    ...
    override suspend fun handle(invocation: ToolInvocation): Result<ToolOutput> {
        ...
        return Result.success(
            ToolOutput.Mcp(
                callResult.fold(
                    onSuccess = { McpResult<io.github.solaceharmony.codex.protocol.CallToolResult, String>(value = it) },
                    onFailure = { McpResult<io.github.solaceharmony.codex.protocol.CallToolResult, String>(error = it.message ?: "Unknown error") }
                )
            )
        )
    }
