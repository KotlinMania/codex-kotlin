...
package ai.solace.coder.tui.color
...
internal fun perceptualDistance(a: Triple<UByte, UByte, UByte>, b: Triple<UByte, UByte, UByte>): Float {
    ...
    fun rgbToXyz(r: UByte, g: UByte, b: UByte): Triple<Float, Float, Float> {
        val rv = srgbToLinear(r)
        val gv = srgbToLinear(g)
        val bv = srgbToLinear(b)

        val x = rv * 0.4124f + gv * 0.3576f + bv * 0.1805f
        val y = rv * 0.2126f + gv * 0.7152f + bv * 0.0722f
        val z = rv * 0.0193f + gv * 0.1192f + bv * 0.9505f
        return Triple(x, y, z)
    }
    ...
    val (l1, la1, lb1) = xyzToLab(x1, y1, z1)
    val (l2, la2, lb2) = xyzToLab(x2, y2, z2)
