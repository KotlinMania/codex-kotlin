...
sealed class TruncationPolicy {
    ...
    companion object {
        ...
        fun approxBytesForTokens(tokens: Int): Int = tokens * APPROX_BYTES_PER_TOKEN

        fun approxTokensFromByteCount(bytes: Int): Int = (bytes + APPROX_BYTES_PER_TOKEN - 1) / APPROX_BYTES_PER_TOKEN
    }
}
...
private fun assembleTruncatedOutput(prefix: String, suffix: String, marker: String): String =
    buildString(prefix.length + marker.length + suffix.length + 1) {
        append(prefix)
        append(marker)
        append(suffix)
    }
