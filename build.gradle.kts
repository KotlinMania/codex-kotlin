...
kotlin {
    ...
    sourceSets {
        commonMain.dependencies {
            ...
            // HTTP types from the workspace sibling port of the hyperium/http
            // crate. We use http-kotlin for the request/response/method/status
            // types instead of pulling in a Ktor client engine; the upstream
            // codex-rs HTTP surface models the same Request/Response shape and
            // the sibling already publishes for every target we configure.
            implementation("io.github.kotlinmania:http-kotlin:0.1.0")

            // Ktor HTTP client core (engine-less; per-target engines wired
            // below). The Curl engine has been dropped because it does not
            // publish for the iOS / tvOS / watchOS / androidNative targets.
            ...
            // log:: levels / entries used across codex-rs state + execpolicy.
        ...
        // io.github.tree-sitter:ktreesitter and ktreesitter-bash only publish
        // for (linuxArm64, linuxX64) — they ship `libtree-sitter` C interop
        // bindings that have no iOS / macOS / mingw / watchOS / tvOS /
        // androidNative variants. Scope the dependency (and the
        // ktreesitter-backed [BashParser]) to `linuxMain`, where the
        // applyDefaultHierarchyTemplate intermediate set groups
        // linuxX64 + linuxArm64.
