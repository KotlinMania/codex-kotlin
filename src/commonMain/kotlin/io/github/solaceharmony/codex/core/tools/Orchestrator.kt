...
import io.github.solaceharmony.codex.core.getErrorMessageUi
    ...
    suspend fun <Req, Out, T> run(
        ...
        when (requirement) {
            ...
            is ApprovalRequirement.NeedsApproval -> {
                var risk: io.github.solaceharmony.codex.protocol.SandboxCommandAssessment? = null
        ...
        val initialSandbox = when (tool.sandboxModeForFirstAttempt(req)) {
            ...
            SandboxOverride.NoOverride -> {
                val pref = when (tool.sandboxPreference()) {
                    SandboxablePreference.Auto -> io.github.solaceharmony.codex.exec.sandbox.SandboxPreference.Auto
                    SandboxablePreference.Require -> io.github.solaceharmony.codex.exec.sandbox.SandboxPreference.Require
                    SandboxablePreference.Forbid -> io.github.solaceharmony.codex.exec.sandbox.SandboxPreference.Forbid
                }
        ...
        return result.fold(
            ...
            onFailure = { err ->
                ...
                if (err is ToolErrorException && err.error is ToolError.Codex) {
                    ...
                    if (sandboxErr !is io.github.solaceharmony.codex.core.SandboxErr.Denied) {
                        return Result.failure(err)
                    }
