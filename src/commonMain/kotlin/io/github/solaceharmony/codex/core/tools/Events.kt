...
import io.github.solaceharmony.codex.protocol.PatchApplyBeginEvent
import io.github.solaceharmony.codex.protocol.PatchApplyEndEvent
import io.github.solaceharmony.codex.protocol.TurnDiffEvent
import io.github.solaceharmony.codex.protocol.parseCommand
...
sealed class ToolEmitter {
    ...
    suspend fun finish(
        ...
        val (event, result) = out.fold(
            ...
            onFailure = { err ->
                when (err) {
                    is io.github.solaceharmony.codex.core.tools.ToolError.Codex -> {
                        when (val inner = err.error) {
                            is CodexErr.Sandbox ->
                                when (val sandboxErr = inner.error) {
                                    is io.github.solaceharmony.codex.core.SandboxErr.Timeout -> {
                                    ...
                                    is io.github.solaceharmony.codex.core.SandboxErr.Denied -> {
                                        val response = formatExecOutputForModel(sandboxErr.output, ctx)
                                        val event = ToolEventStage.Failure(ToolEventFailure.Output(sandboxErr.output))
                                        val result = Result.failure<String>(FunctionCallError.RespondToModel(response))
                                        Pair(event, result)
                    ...
                    is io.github.solaceharmony.codex.core.tools.ToolError.Rejected -> {
                        val msg = err.reason
